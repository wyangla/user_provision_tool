...
register.py -u alice -sn myapp
            -pr myapp                  ← bare name: resolved to SOURCE_PROJECTS_DIR/myapp
                                         = $PROVISION_DIR/source_projects/myapp
            -fc docker-compose.prod.yml  ← filename relative to resolved -pr
            -fn nginx.prod.conf          ← filename relative to resolved -pr
...
Nginx conf and .htpasswd files are written into GENERATED_DIR
  = $PROVISION_DIR/generated by default.

SOURCE_PROJECTS_DIR = $PROVISION_DIR/source_projects  (default; override via env var)
GENERATED_DIR       = $PROVISION_DIR/generated        (default; override via env var)

source_project/service_1/          ← project root (-pr), resolved from bare name
                                     or given as full/relative path
...
```
-pr  myapp                          bare name  → SOURCE_PROJECTS_DIR/myapp
                                               = $PROVISION_DIR/source_projects/myapp
     source_project/service_1       relative/absolute path used as-is
...
= SOURCE_PROJECTS_DIR/myapp      nginx.prod.conf.j2              ← converted template (step 0b)
= $PROVISION_DIR/source_projects/myapp
...
= $PROVISION_DIR/generated       myapp.user-alice.0.htpasswd     ← bcrypt password     (step 3/5)
